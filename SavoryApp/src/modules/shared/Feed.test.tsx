...
//           if (url === `https://savory-backend.azurewebsites.net/api/posts/allWithUsername?pageNumber=${pageNumber}`) {
...
describe('Feed', () => {
    ...
    beforeEach(() => {
        ...
        (global as any).fetch = jest.fn().mockImplementation((input: RequestInfo | URL, init?: RequestInit) => 
        {
          ...
          if (url === `https://savory-backend.azurewebsites.net/api/posts/allWithUsername?pageNumber=1`) 
          {
                  console.log("In Statement")      
                  return Promise.resolve({
